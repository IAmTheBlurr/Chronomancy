...
class Chronomancy:
    """ A class to help with time manipulation and other time related tasks. """
    def __init__(self):
        # Capture the calling frame at initialization time
        self.anchor_frame = inspect.stack()[2]

    def arcane_recall(self, target_argument_pos=0):
        """
        ...
        calling_code = self.anchor_frame.code_context[0].strip('\n')
        calling_locals = self.anchor_frame.frame.f_locals
        ...
        top_level_object = self.anchor_frame.frame.f_locals[top_level_target_id]
        ...
        for index, command in enumerate(dot_notation_chain):
            ...
            if inspect.isclass(type(getattr(current_level_object, command))) or callable(
                    getattr(current_level_object, command)):
